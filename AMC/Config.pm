...
# perl -le 'use Gtk3 -init; print Gtk3::Gdk::Color::parse("blue")->to_string()'
    ...
    $self->{o_default} = {
        ...
        img_viewer => [
            'command', 'eog', 'ristretto', 'gpicview',
            'mirage',  'gwenview',
        ],
        csv_viewer => [
        ...
        dir_opener => [
            'command', 'nautilus --no-desktop file://%d',
            'pcmanfm %d', 'Thunar %d', 'konqueror file://%d',
            'dolphin %d',
        ],
...
# TRANSLATORS: Body text of the emails which can be sent to the students to give them their annotated completed answer sheet.
    ...
    if ( lc($^O) eq 'darwin' ) {
        ...
        {
            $self->{o_default}->{$k}
                = [ 'commande', '/usr/bin/open', 'open' ];
        }
        $self->{o_default}->{html_browser}
            = [ 'commande', '/usr/bin/open %u', 'open %u' ];
    }
...
sub test_commands {
    ...
    if ( @uc && !$dont_warn ) {
        if ( $self->{gui} ) {
            my $dialog = Gtk3::MessageDialog->new( $self->{gui},
...
sub parse_key {
    ...
    if ( $k->{container} ) {
        $k->{location} = path_end( $self->{ $k->{container} }, $create,
            @{ $k->{path} } );
    }
    return ($k);
}
